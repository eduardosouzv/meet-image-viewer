(() => {
  ...
  function handleChatMutation(mutations) {
    mutations.forEach((mutation) => {
      _safeArrayFromNodeList(mutation.addedNodes).forEach((addedNode) => {
        ...
        if (addedNode.children) {
          allAddedElements = [...allAddedElements, flattenChildrenElements(addedNode)];
        }
        allAddedElements = allAddedElements.flat();

        const addedMessages = allAddedElements.filter(
          (addedElement) =>
            addedElement &&
            typeof addedElement.hasAttribute === 'function' &&
            addedElement.hasAttribute('data-message-text')
        );

        addedMessages.forEach((messageNode) => {
          if (messageNode.textContent && messageNode.textContent.startsWith('https')) {
    ...
    function replaceMessageContent(messageNode) {
      ...
      img.onload = function () {
        messageNode.replaceChildren(img);
      };
      img.onerror = function () {
        img.remove();
      };
      img.src = messageNode.textContent;
    }
    ...
    function flattenChildrenElements(parentNode) {
      return _safeArrayFromNodeList(parentNode.children).reduce((allChildren, element) => {
